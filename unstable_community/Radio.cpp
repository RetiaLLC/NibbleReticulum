...
sx126x::sx126x(uint8_t index, SPIClass* spi, bool tcxo, bool dio2_as_rf_switch, int ss, int sclk, int mosi, int miso, int reset, int dio0, int busy, int rxen) :
  RadioInterface(index),
    _spiSettings(16E6, MSBFIRST, SPI_MODE0), _spiModem(spi), _ss(ss),
...
bool sx126x::preInit() {
  ...
  while (((millis() - start) < 2000) && (millis() >= start)) {
      ...
      Serial.print("DBG: syncmsb="); Serial.print(syncmsb, HEX);
      Serial.print(" synclsb="); Serial.println(synclsb, HEX);
